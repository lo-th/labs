...
V.BvhPlayer.prototype={constructor:V.BvhPlayer,init:function(){},skin:function(a,b){null!==this.model&&this.clearSkin();this.model=a;this.bones=this.model.skeleton.bones;this.boneInverses=this.model.skeleton.boneInverses;this.preservesBoneSize=b||!1;this.root.scene.add(this.model)},clearSkin:function(){this.root.scene.remove(this.model)},boneSize:function(a){a/=30;0>a&&(a=0.1);this.reader.boneSize=a},speed:function(a){this.reader.speed=a||1},load:function(a,b){this.reader.load(a,b)},update:function(){this.reader.update();
null!==this.model&&this.updateSkin()},updateSkin:function(){for(var a,b,c,d=this.reader.Nodes,e=this.bones.length,f,g,k,l=new THREE.Quaternion,h=new THREE.Vector3,n=new THREE.Vector3,m=0;m<e;m++)a=this.bones[m],c=a.name,k=a.parent.matrixWorld||new THREE.Matrix4,g=a.parent.mtx?a.parent.mtx:k,(b=d[c])?(f=b.matrixWorld.clone(),h.setFromMatrixPosition(f),l.setFromRotationMatrix(f),b=new THREE.Matrix4,a.rootMatrix||(a.rootMatrix=a.matrixWorld.clone()),b.makeRotationFromQuaternion(l),b.multiply(a.rootMatrix),
b.setPosition(h),f=(new THREE.Matrix4).getInverse(k),f=(new THREE.Matrix4).multiplyMatrices(f,b),b.multiplyMatrices(k,f),this.preservesBoneSize&&"Hips"!==c&&(f=(new THREE.Matrix4).getInverse(g),n.setFromMatrixPosition(a.matrix),f=(new THREE.Matrix4).multiplyMatrices(f,b),f.setPosition(n),b=(new THREE.Matrix4).multiplyMatrices(g,f))):b=(new THREE.Matrix4).multiplyMatrices(g,a.matrix),a.mtx=b}};
