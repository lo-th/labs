'use strict';var LTH={rubriques:[],demoNames:[],libsNames:[],cRubr:-1,cFile:0},esprima=esprima||{};esprima.parse=esprima.parse||{};var CodeMirror=CodeMirror||{},GRAD=GRAD||{},history,main,transcode;window.onload=init;function init(){transcode=new LTH.Transcode("full serious rot liquidfun ammo oimo crowd".split(" "),endTranscode,!0)}function endTranscode(){main=new LTH.Main}
...
transcode.isWebGl?(this.initLogo(!0),this.initHome()):this.initLogo(!1);var b=this;this.main.happ&&window.addEventListener("popstate",function(a){b.isHome?history.back():b.initHome(a)})};
LTH.Menu.prototype={constructor:LTH.Menu,initLogo:function(a){this.logo=this.doc.createElement("div");this.logo.className="logo";this.content.appendChild(this.logo);this.logo.innerHTML=LTH.Logos(this.baseColor);this.title=this.doc.createElement("div");this.title.className="title";this.content.appendChild(this.title);a?(this.title.innerHTML="LOTH LABS",this.logo.onmousedown=function(a){this.initHome(a)}.bind(this),this.logo.onmouseover=function(a){this.isHome||(this.title.innerHTML="BACK HOME")}.bind(this),
this.logo.onmouseout=function(a){this.isHome||(this.title.innerHTML=LTH.rubriques[LTH.cRubr].toUpperCase())}.bind(this)):this.title.innerHTML="SORRY, YOU NEED WEBGL BROWSER";this.miniLogos()},miniLogos:function(){this.micrologo=this.doc.createElement("div");this.micrologo.className="minilogoOut";this.content.appendChild(this.micrologo);this.iconsOut=[];for(var a=4;a--;){this.iconsOut[a]=this.doc.createElement("div");this.iconsOut[a].className="mmmlogo";this.iconsOut[a].name=a;var b=document.createElement("div");
...
LTH.FileSystem.prototype={constructor:LTH.FileSystem,loadXML:function(a){var b;window.XMLHttpRequest?b=new XMLHttpRequest:window.ActiveXObject&&(b=new ActiveXObject("Microsoft.XMLHTTP"));b.onload=function(a){a=b.responseXML.getElementsByTagName("rubrique");for(var d,e=0;e<a.length;e++){d=a[e];LTH.rubriques[e]=d.attributes[0].value;LTH.demoNames[e]=[];LTH.libsNames[e]=[];d=d.getElementsByTagName("demo");for(var f=0;f<d.length;f++)LTH.demoNames[e][f]=d[f].attributes[0].value,LTH.libsNames[e][f]=d[f].attributes[1].value.split("|").map(function(a){return a})}this.main.init()}.bind(this);
...
0;0!==a[1]&&(b=1);0!==a[2]&&(b=2);for(var c,d="",e=a.length/4,f=0;f<e;f++)c=4*f+b,c=a[c]+32,c=127==c?10:c,d+=String.fromCharCode(c);this.isDebug&&console.log(this.name,Date.now()-this.time+"ms");"full"==this.name&&(d.substring(3,8),this.useTrans=!1);try{var g=new Blob([d],{type:"application/javascript"});this.codes[this.name]=URL.createObjectURL(g);this.useTrans=!0}catch(h){this.useTrans=!1}this.useTrans||(this.list.length=0)},createProgram:function(a,b){var c=a.createShader(a.FRAGMENT_SHADER);a.shaderSource(c,
b.fs);a.compileShader(c);var d=a.createShader(a.VERTEX_SHADER);a.shaderSource(d,b.vs);a.compileShader(d);var e=a.createProgram();a.attachShader(e,d);a.attachShader(e,c);a.linkProgram(e);a.getProgramParameter(e,a.LINK_STATUS)||console.error("Error linking the shader: "+a.getProgramInfoLog(e));return e}};LTH.BitmapShader={fs:"precision mediump float;\nuniform sampler2D u_image;\nvarying vec2 v_texCoord;\nvoid main() { gl_FragColor = texture2D(u_image, v_texCoord); }",vs:"attribute vec2 a_position;\nattribute vec2 a_texCoord;\nuniform vec2 u_resolution;\nvarying vec2 v_texCoord;\nvoid main() {\nvec2 zeroToOne = a_position / u_resolution;\nvec2 zeroToTwo = zeroToOne * 2.0;\nvec2 clipSpace = zeroToTwo - 1.0;\ngl_Position = vec4(clipSpace * vec2(1, -1), 0, 1);\nv_texCoord = a_texCoord;\n}"};
