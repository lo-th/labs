...
rimColor:    { type:'c', value: new THREE.Color(0x323436) },

useNormal:   { type:'f', value: 0.0 },
...
viewSize:    { type:'v2', value: new THREE.Vector2(512,512) },
mapSize:     { type:'v2', value: new THREE.Vector2(1024,1024) },
mapType:     { type:'f', value: 1.0 },

reflectionFactor: { type:'f', value: 0.5 },
refractionRatio: { type:'f', value: 0.98 }
},
fs: [
'uniform sampler2D spherical;',
...
'void main() {',
	'vec2 b = vec2(1.0/viewSize.x, 1.0/viewSize.y);',
	'vec4 texel = blurred(tDiffuse, vUv, b);',
	'vec4 texelMin = blurred(tDiffuseMin, vUv, b);',
	'float aspR = viewSize.x/viewSize.y;',
	'vec2 uvMap = (vUv);',

	'if(mapType!=3.0){',
		'vec2 resolution = vec2(1.0, 1.0)/viewSize;',
	    'vec2 mapResolution = mapSize*resolution;',
	    'vec2 mid = ((viewSize*0.5)/(mapSize*0.5));',
	...
	'vec4 reflectMap = texture2D( spherical, project );',
	...
	'if( useNormal == 1. ) {',
		...
		'reflectMap = texture2D( spherical, calculatedNormal );',
	'}',
	...
	'}',
	'if(seriousType == 1){',
	    'vec2 sresolution = vec2(1.0, 1.0)/viewSize;',
	    'vec2 smapResolution =  seriousSize*sresolution;',
	    'vec2 smid = ((viewSize*0.5)/( seriousSize*0.5));',
