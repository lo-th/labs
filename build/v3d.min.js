...
V.BvhPlayer.prototype={constructor:V.BvhPlayer,init:function(){},skin:function(a){null!==this.model&&this.clearSkin();this.model=a;this.bones=this.model.skeleton.bones;this.boneInverses=this.model.skeleton.boneInverses;this.preservesBoneSize=!0;this.root.scene.add(this.model)},clearSkin:function(){},boneSize:function(a){a/=30;0>a&&(a=0.1);this.reader.boneSize=a},speed:function(a){this.reader.speed=a||1},load:function(a,b){this.reader.load(a,b)},update:function(){this.reader.update();null!==this.model&&
this.updateSkin()},updateSkin:function(){var a,b,c=this.reader.Nodes,d=this.bones.length,e=new THREE.Vector3,f=new THREE.Quaternion,g=new THREE.Matrix4;b=new THREE.Matrix4;for(var h=d-1;h--;)a=this.bones[h],b=a.name,(b=c[b])?(b=(new THREE.Matrix4).copy(b.matrixWorld),new THREE.Vector3,e.setFromMatrixPosition(b),f.setFromRotationMatrix(b),g=new THREE.Matrix4,a.rootMatrix||(a.rootMatrix=a.matrixWorld.clone()),g.makeRotationFromQuaternion(f),g.multiply(a.rootMatrix),g.setPosition(e),b=(new THREE.Matrix4).getInverse(a.parent.matrixWorld),
b=(new THREE.Matrix4).multiplyMatrices(b,g),g.multiplyMatrices(a.parent.matrixWorld,b),a.mtx=g.clone()):(g=(new THREE.Matrix4).multiplyMatrices(a.parent.mtx?a.parent.mtx:a.parent.matrixWorld,a.matrix),a.mtx=g);if(this.preservesBoneSize)for(h=0;h<d;h++)a=this.bones[h],b=a.name,"Hips"!==b&&(e=a.parent.mtx?a.parent.mtx:a.parent.matrixWorld,c[b]?(b=new THREE.Matrix4,f=new THREE.Vector3,b.getInverse(e),b=(new THREE.Matrix4).multiplyMatrices(b,a.mtx),f.setFromMatrixPosition(a.matrix),b.setPosition(f),g=
(new THREE.Matrix4).multiplyMatrices(e,b)):g=(new THREE.Matrix4).multiplyMatrices(e,a.matrix),a.mtx=g)}};THREE.Skeleton.prototype.update=function(){var a=new THREE.Matrix4;return function(){for(var b=this.bones.length;b--;)this.bones[b].mtx||(this.bones[b].mtx=this.bones[b].matrixWorld),a.multiplyMatrices(this.bones[b]?this.bones[b].mtx:this.identityMatrix,this.boneInverses[b]),a.flattenToArrayOffset(this.boneMatrices,16*b);this.useVertexTexture&&(this.boneTexture.needsUpdate=!0)}}();
