...
vs:"attribute vec4 tangent;\nuniform vec2 uRepeatBase;\nuniform sampler2D tNormal;\nuniform sampler2D tDisplacement;\nuniform float uDisplacementScale;\nuniform float uDisplacementBias;\nvarying vec3 vTangent;\nvarying vec3 vBinormal;\nvarying vec3 vNormal;\nvarying vec2 vUv;\nvarying vec3 vViewPosition;\nvarying vec3 vWorldPosition;\nvarying float vAmount;\nvarying vec2 vN;\nvarying vec3 vPos;\nvoid main() {\nvNormal = normalize( normalMatrix * normal );\nvTangent = normalize( normalMatrix * tangent.xyz );\nvBinormal = cross( vNormal, vTangent ) * tangent.w;\nvBinormal = normalize( vBinormal );\nvUv = uv;\nvec4 bumpData = texture2D( tDisplacement, uv );\nvAmount = bumpData.r;\nvec2 uvBase = uv * uRepeatBase;\nvec3 dv = texture2D( tDisplacement, uvBase ).xyz;\nfloat df = uDisplacementScale * dv.x + uDisplacementBias;\nvec3 displacedPosition = normal * df + position;\nvec4 worldPosition = modelMatrix * vec4( displacedPosition, 1.0 );\nvec4 mvPosition = modelViewMatrix * vec4( displacedPosition, 1.0 );\ngl_Position = projectionMatrix * mvPosition;\nvWorldPosition = worldPosition.xyz;\nvViewPosition = -mvPosition.xyz;\nvec3 normalTex = texture2D( tNormal, uvBase ).xyz * 2.0 - 1.0;\nvNormal = normalMatrix * normalTex;\nvPos = normalize( vec3( mvPosition ) );\nvec3 r = reflect( vPos, normalize(vNormal) );\nfloat m = 2. * sqrt( pow( r.x, 2. ) + pow( r.y, 2. ) + pow( r.z + 1., 2. ) );\nvN = r.xy / m + .5;\n}"};V.Worker=function(a,b){this.root=a;this.name=b;this.msg="";this.postMess=this.update=null;this.testNewAmmo=window.top.main.useDirectDebug;var c,d,e,f,g,h,l,k;switch(this.name){case "crowd":c="js/worker/crowd_worker.js";k="../../js/libs/crowd.js";d=1E3;e=100;f=10;g=1E3;h=3;l=2;this.update=this.upCrowd;this.postMess=this.postCrowd;break;case "liquid":c="js/worker/liquid_worker.js";k="../../js/libs/liquidfun.js";d=1E3;e=100;f=10;g=8E3;h=4;l=2;this.update=this.upLiquid;this.postMess=this.postLiquid;break;
case "oimo":c="js/worker/oimo_worker.js";k="../../js/libs/oimo.min.js";d=1E3;f=e=10;g=0;h=8;l=3;this.update=this.upOimo;this.postMess=this.postOimo;break;case "ammo":this.testNewAmmo?(c="js/worker/ammo_worker_new.js",k="../../js/libs/ammo.js"):(c="js/worker/ammo_worker.js",k="../../js/libs/ammo.min.js");d=1E3;e=20;f=10;g=0;h=8;l=40;this.update=this.upAmmo;this.postMess=this.postAmmo;break;case "traffic":c="js/worker/traffic_worker.js",k="../../js/libs/traffic.min.js",d=1E3,f=e=10,g=0,l=h=3,this.update=
this.upTraffic,this.postMess=this.postTraffic}window.top.main.transcode.useTrans&&(k=window.top.main.transcode.codes[this.name]);this.w=new Worker(c);this.w.postMessage=this.w.webkitPostMessage||this.w.postMessage;this.w.onmessage=function(a){this.update(a)}.bind(this);this.ar=new V.AR32(d*h);this.dr=new V.AR32(e*l);this.pr=new V.AR32(g*l);this.drn=new V.AR32(f);this.drc=new V.AR32(f);this.prn=new V.AR32(f);this.d=[16.667,0,0];this.isReady=!1;this.fps=0;this.w.postMessage({m:"init",url:k})};
...
V.getFaces=function(a,b){for(var c=[],d,e,f,g,h=a.faces,l=a.vertices,k=h.length;k--;)d=9*k,e=h[k],f=l[e.a],g=l[e.b],e=l[e.c],c[d+0]=f.x*b[0],c[d+1]=f.y*b[1],c[d+2]=f.z*b[2],c[d+3]=g.x*b[0],c[d+4]=g.y*b[1],c[d+5]=g.z*b[2],c[d+6]=e.x*b[0],c[d+7]=e.y*b[1],c[d+8]=e.z*b[2];return c};V.BvhPlayer=function(a){this.root=a;this.reader=new V.BvhReader(this.root);this.model=null};
V.BvhPlayer.prototype={constructor:V.BvhPlayer,init:function(){},skin:function(a){null!==this.model&&this.clearSkin();this.model=a;this.bones=this.model.skeleton.bones;this.boneInverses=this.model.skeleton.boneInverses;this.preservesBoneSize=!0;for(a=this.bones.length;a--;)console.log(this.bones[a].name);this.root.scene.add(this.model)},clearSkin:function(){this.root.scene.remove(this.model);this.model.material.dispose();this.model.geometry.dispose()},boneSize:function(a){a/=30;0>a&&(a=0.1);this.reader.boneSize=
a},speed:function(a){this.reader.speed=a||1},load:function(a,b){this.reader.load(a,b)},update:function(){this.reader.update();null!==this.model&&this.updateSkin()},updateSkin:function(){var a,b,c=this.reader.Nodes,d=this.bones.length,e=new THREE.Vector3,f=new THREE.Quaternion,g=new THREE.Matrix4;b=new THREE.Matrix4;for(var h=d;h--;)a=this.bones[h],b=a.name,(b=c[b])?(b=(new THREE.Matrix4).copy(b.matrixWorld),new THREE.Vector3,e.setFromMatrixPosition(b),f.setFromRotationMatrix(b),g=new THREE.Matrix4,
a.rootMatrix||(a.rootMatrix=a.matrixWorld.clone()),g.makeRotationFromQuaternion(f),g.multiply(a.rootMatrix),g.setPosition(e),b=(new THREE.Matrix4).getInverse(a.parent.matrixWorld),b=(new THREE.Matrix4).multiplyMatrices(b,g),g.multiplyMatrices(a.parent.matrixWorld,b),a.mtx=g.clone()):(g=(new THREE.Matrix4).multiplyMatrices(a.parent.mtx?a.parent.mtx:a.parent.matrixWorld,a.matrix),a.mtx=g);if(this.preservesBoneSize)for(h=d;h--;)a=this.bones[h],b=a.name,"Hips"!==b&&(d=a.parent.mtx?a.parent.mtx:a.parent.matrixWorld,
c[b]?(f=new THREE.Matrix4,e=new THREE.Vector3,f.getInverse(d),b=(new THREE.Matrix4).multiplyMatrices(f,a.mtx),e.setFromMatrixPosition(a.matrix),b.setPosition(e),g=(new THREE.Matrix4).multiplyMatrices(d,b)):g=(new THREE.Matrix4).multiplyMatrices(d,a.matrix),a.mtx=g)}};
