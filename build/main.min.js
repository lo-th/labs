...
(this.current=null,this.title.style.background="#"+this.topColor,this.title.innerHTML="",this.subTitle.innerHTML="")},findRubrique:function(a){for(var b=this.nRubriques;b--;)a<this.anglesEnd[b]&&a>this.anglesStart[b]&&(this.current=this.rubNames[b])},compute:function(){var a=0,b,c=0,d;for(d in this.arcs)b=this.arcs[d],this.colors[a]=b.color,this.demosNames[a]=b.demos,this.demosIcones[a]=b.icon,this.nDemos[a]=b.demos.length,c+=this.nDemos[a],a++;b=360/c;var e=c=0,a=0;for(d in this.arcs){c=this.nDemos[a]*
b;this.rubNames[a]=d;this.anglesStart[a]=e+this.decal+this.rdecal;this.anglesEnd[a]=e+c-this.decal+this.rdecal;var f=this.anglesStart[a]+0.5*(this.anglesEnd[a]-this.anglesStart[a])-60;this.angles[a]=f;this.points[a]=this.findPoint(f);e+=c;a++}this.nRubriques=a;this.isComputed=!0},findPoint:function(a){var b=a*LTH.ToRad,c=(a+13)*LTH.ToRad;a=(a-13)*LTH.ToRad;for(var d,e=90,f=this.endRadius,g=[],h=0;13>h;h++)0==h?d=this.radialPos(b,f+25):1==h?d=this.radialPos(b,f+70):1==(h&1)?(d=this.radialPos(a,f+e),
e+=45):d=this.radialPos(c,f+e),g.push(d);return g},radialPos:function(a,b){return{x:Math.cos(a)*b+this.center,y:Math.sin(a)*b+this.center}},draw:function(a){this.ctx.clearRect(0,0,this.size,this.size);for(var b=this.nRubriques;b--;)this.ctx.strokeStyle=this.colors[b],this.ctx.beginPath(),this.ctx.arc(this.center,this.center,this.radius,(this.anglesStart[b]-60)*LTH.ToRad,(this.anglesEnd[b]-60)*LTH.ToRad,!1),this.ctx.lineWidth=this.line,this.ctx.stroke();a&&this.placesPins()},placesPins:function(){this.prevPins=
[];this.startPins=0;this.endPins=13;this.time=100;this.anim=setTimeout(function(a){this.updatePins()}.bind(this),this.time)},updatePins:function(){var a=this.nRubriques,b,c,d;if(this.startPins==this.endPins)clearTimeout(this.anim);else{for(b=this.prevPins.length;b--;)this.prevPins[b].style.display="block";for(b=this.pins.length;b--;)this.pins[b].className="pins pinson",this.prevPins[b]=this.pins[b].childNodes[0];for(;a--;)b=this.points[a],c=this.nDemos[a],d=this.colors[a],this.startPins<c&&(this.liner(b,
d,this.startPins),this.addPins(b[this.startPins+1].x,b[this.startPins+1].y,d,this.startPins,a));this.startPins++;this.anim=setTimeout(function(a){this.updatePins()}.bind(this),this.time)}},liner:function(a,b,c){this.ctx.strokeStyle=b;this.ctx.beginPath();this.ctx.moveTo(a[0].x,a[0].y);0==c&&this.ctx.lineTo(a[1].x,a[1].y);1==c&&(this.ctx.moveTo(a[1].x,a[1].y),this.ctx.lineTo(a[2].x,a[2].y));2==c&&(this.ctx.moveTo(a[1].x,a[1].y),this.ctx.lineTo(a[3].x,a[3].y));3==c&&(this.ctx.moveTo(a[2].x,a[2].y),
this.ctx.lineTo(a[4].x,a[4].y));4==c&&(this.ctx.moveTo(a[3].x,a[3].y),this.ctx.lineTo(a[5].x,a[5].y));5==c&&(this.ctx.moveTo(a[4].x,a[4].y),this.ctx.lineTo(a[6].x,a[6].y));6==c&&(this.ctx.moveTo(a[5].x,a[5].y),this.ctx.lineTo(a[7].x,a[7].y));this.ctx.lineWidth=2;this.ctx.stroke()},addPins:function(a,b,c,d,e){var f=this.pins.length,g=document.createElement("div");g.className="pins";g.style.left=a+"px";g.style.top=b+"px";g.style.background=c;g.name=e+"_"+d;a=document.createElement("div");a.className=
"pinsin";a.innerHTML=LTH.IconMicro(this.topColor,this.demosIcones[e][d]);g.appendChild(a);this.pins[f]=g;this.pins[f].onclick=function(a){var b=a.target.name.substring(0,1);a=a.target.name.substring(2,a.target.name.length);this.main.menu.resetHome(b,a)}.bind(this);this.pins[f].onmouseover=function(a){a.target.style.background="#"+this.baseColor;var b=a.target.name.substring(0,1);a=a.target.name.substring(2,a.target.name.length);this.subTitle.innerHTML=this.demosNames[b][a].replace("_"," ").toUpperCase()}.bind(this);
...
break;case 6:c[1]="<path fill='#"+a+"' d='M 25 8.5 L 20.7 4.05 6 9 4 10 4 18 6 22 10 26 25 20 25 8.5 M 8.4 10.35 L 19.8 6.45 22.6 9.05 10.6 12.7 8.4 10.35 M 6 11.7 L 7 11 10 14 10 23 7.75 20.8 6 17 6 11.7 M 23 10 L 23 18.4 11 23 11 13.65 23 10 M 21.6 17.8 L 21.6 11.75 12.5 14.65 12.5 21.2 21.6 17.8 M 20.75 13.05 L 20.75 16.95 13.6 19.65 13.6 15.3 20.75 13.05 Z'/>";break;case 7:c[1]="<path fill='#"+a+"' d='M 16 11 L 14 11 14 14 11 14 11 16 14 16 14 19 16 19 16 16 19 16 19 14 16 14 16 11 Z'/>";break;
case 8:c[1]="<path fill='#"+a+"' d='M 16 11 L 14 11 14 14 11 14 11 16 14 16 14 19 16 19 16 16 19 16 19 14 16 14 16 11 Z'/>"}c[2]="<use xlink:href='#Layer'/></g></svg>";return c.join("\n")};LTH.logoTimer=null;
