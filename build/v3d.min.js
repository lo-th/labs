...
V.View=function(a,b,c,d){this.renderMode="";window.top.main.previewTheme();this.dimentions={w:window.innerWidth,h:window.innerHeight,r:window.innerWidth/window.innerHeight};this.mem="";this.canvas=canvas;this.debug=debug;this.info=info;this.loader=loader;this.renderer=new THREE.WebGLRenderer({canvas:canvas,precision:"mediump",antialias:!0,alpha:!0,stencil:!1});this.renderer.setSize(this.dimentions.w,this.dimentions.h);this.renderer.setPixelRatio(window.devicePixelRatio);this.renderer.setClearColor(0,
0);this.renderer.autoClear=!1;this.clock=new THREE.Clock;this.z=null;this.zones=[];this.scene=new THREE.Scene;this.sceneBlob=new THREE.Scene;this.nav=new V.Nav(this,a,b,c);this.base=new THREE.Group;this.content=new THREE.Group;this.skinned=new THREE.Group;this.scene.add(this.base);this.scene.add(this.content);this.scene.add(this.skinned);this.environment=null;this.initGeo();this.initMat();this.pool=new V.SeaPool(this);this.impool=new V.ImgPool(this);d&&this.loadBasic();this.postEffect=null;this.deb=
"";this.f=[0,0,0,0];this.meshs=[];this.cars=[];this.speeds=[];this.wheels=[];this.steering=[];this.lines=[];this.anchors={};this.ps=[];this.model="skull";this.w=this.groundMirror=this.basic=null;this.isWithSerious=this.isW=!1;window.onresize=function(a){this.resize(a)}.bind(this)};
V.View.prototype={constructor:V.View,render:function(){this.w&&!this.isW&&this.w.isReady&&(this.wFun(),this.isW=!0);THREE.AnimationHandler.update(this.clock.getDelta());this.isWithSerious&&this.renderer.resetGLState();TWEEN&&TWEEN.update();this.groundMirror&&this.groundMirror.render();null!==this.postEffect&&this.postEffect.isActive?this.postEffect.render():this.renderer.render(this.scene,this.nav.camera);var a=this.f;a[0]=Date.now();a[0]-1E3>a[1]&&(a[1]=a[0],a[3]=a[2],a[2]=0);a[2]++;this.debug.innerHTML=
"THREE "+a[3]+this.deb},bytesToSize:function(a,b){if(0==a)return"n/a";var c=Math.pow(10,void 0!==b?b:0),d=Math.floor(Math.log(a)/Math.log(1024));return Math.round(a*c/Math.pow(1024,d))/c+" "+["bytes","kb","mb","gb","tb"][d]},initGui:function(a){this.gui=new V.Gui(a)},ssao:function(a){this.renderMode="ssao";this.postEffect=new V.PostEffect(this,"ssao",a)},metaball:function(a){this.renderMode="metaball";this.postEffect=new V.PostEffect(this,"metaball",!1,a)},distortion:function(){this.renderMode="distortion";
this.postEffect=new V.PostEffect(this,"distortion",!1)},deformSsao:function(a,b){this.postEffect.deformSsao(a,b)},mirror:function(a,b){a=a||100;b=b||{};this.groundMirror=new THREE.Mirror(v.renderer,v.nav.camera,{clipBias:b.bias||0.003,textureWidth:this.dimentions.w,textureHeight:this.dimentions.h,color:b.color||7829367,alpha:b.alpha||0.02,power:b.power||1,radius:b.radius||1});var c=new THREE.Mesh(this.geo.plane,this.groundMirror.material);c.scale.set(a,a,a);c.position.copy(b.pos||new THREE.Vector3);
c.rotateX(-V.PI90);c.add(this.groundMirror);this.scene.add(c)},resize:function(){this.dimentions.w=window.innerWidth;this.dimentions.h=window.innerHeight;this.dimentions.r=this.dimentions.w/this.dimentions.h;this.renderer.setSize(this.dimentions.w,this.dimentions.h);this.nav.camera.aspect=this.dimentions.r;this.nav.camera.updateProjectionMatrix();null!==this.groundMirror&&this.groundMirror.resize(this.dimentions.w,this.dimentions.h,v.nav.camera);null!==this.postEffect&&this.postEffect.isActive&&this.postEffect.resize(this.dimentions.w,
this.dimentions.h)},colorBack:function(a){null!==this.postEffect&&this.renderer.setClearColor(a,1)},zone:function(a){a=a||{};var b=this.zones.length,c=a.pos||[0,0,0],d=a.s||1,e=new THREE.Mesh(this.geo[a.type||"ground"],this.mat.zone);e.name="zone"+b;e.scale.set(d,d,d);e.position.set(c[0],c[1],c[2]);e.visible=a.v||!1;this.base.add(e);this.zones[b]=e},addModel:function(a){null!==this.basic&&this.scene.remove(this.basic);"plane"===this.model?(this.basic=new THREE.Mesh(this.geo.ground,a),this.basic.geometry.computeTangents(),
...
2,-2):"head"==v.model?(v.basic.scale.set(8,8,-8),v.basic.position.y=-40):"woman"==v.model?(v.basic.scale.set(60,60,-60),v.basic.position.y=-37):v.basic.scale.set(1,1,-1);v.scene.add(v.basic)},loadBasic:function(){this.pool.load("object",this.initObject,!1,!0)},initObject:function(){v.geo.box=(new THREE.BufferGeometry).fromGeometry(v.pool.getGeometry("object","box"));v.geo.cylinder=(new THREE.BufferGeometry).fromGeometry(v.pool.getGeometry("object","cylinder"))},basic:function(){this.basic=new THREE.Mesh(this.geo.ground,
this.mat.base);this.basic.scale.set(50,50,50);this.scene.add(this.basic)},ground:function(){this.m=new THREE.Mesh(this.geo.ground,this.mat.base);this.m.scale.set(50,50,50);this.scene.add(this.m)},tell:function(a){this.info.innerHTML=a},addBlob:function(a,b,c){a=new V.Blob(this,a,b);if(c)return a;this.meshs[this.meshs.length]=a},addSolid:function(a){var b=new THREE.Mesh(this.geo[a.type||"box"],this.mat.base);a.pos=a.pos||[0,0,0];a.size=a.size||[1,1,1];b.scale.set(a.size[0],a.size[1],a.size[2]);b.position.set(a.pos[0],
a.pos[1],a.pos[2]);b.rotation.set(0,0,0);this.scene.add(b);b.visible=!1;b=new THREE.BoxHelper(b);b.material=this.mat[a.mat||"chaine"];this.scene.add(b)},add:function(a){var b;a.pos=a.pos||[0,0,0];a.size=a.size||[1,1,1];"blob"==a.type?(b=new THREE.Vector3(a.pos[0],a.pos[1],a.pos[2]),b=new V.Blob(this,b,4*a.size[0]),this.sceneBlob.add(b)):(b=new THREE.Mesh(this.geo[a.type||"box"],this.mat[a.mat||"shad_"+a.type]),b.scale.set(a.size[0],a.size[1],a.size[2]),b.position.set(a.pos[0],a.pos[1],a.pos[2]),b.rotation.set(0,
0,0),this.scene.add(b));this.meshs[this.meshs.length]=b;this.w&&this.w.add(a)},addParticle:function(a){var b=this.ps.length;this.ps[b]=new V.Particle(this,a);a.id=b;this.w&&this.w.addParticle(a)},initGeo:function(){var a=new THREE.SphereGeometry(1,34,28);a.computeTangents();var b={};b.sphere=(new THREE.BufferGeometry).fromGeometry(new THREE.SphereGeometry(1,12,10));b.sphereHigh=a;b.box=(new THREE.BufferGeometry).fromGeometry(new THREE.BoxGeometry(1,1,1));b.cylinder=(new THREE.BufferGeometry).fromGeometry(new THREE.CylinderGeometry(1,
1,1,12,1));b.plane=new THREE.PlaneBufferGeometry(1,1);b.ground=new THREE.PlaneBufferGeometry(1,1);b.ground.applyMatrix((new THREE.Matrix4).makeRotationX(-V.PI90));this.geo=b},initMat:function(){this.environment=THREE.ImageUtils.loadTexture("images/spherical/e_chrome.jpg");this.environment.mapping=THREE.SphericalReflectionMapping;var a={};a.none=new THREE.MeshBasicMaterial({transparent:!0,opacity:0,fog:!1,depthTest:!1,depthWrite:!1});a.zone=new THREE.MeshBasicMaterial({color:65280,transparent:!0,opacity:0.1,
fog:!1,depthTest:!1,depthWrite:!1});a.base=new THREE.MeshBasicMaterial({color:0});a.anchor=new THREE.MeshBasicMaterial({color:16711795});a.chaine=new THREE.LineBasicMaterial({color:16711795,transparent:!0,opacity:0.3});a.Sanchor=new THREE.MeshBasicMaterial({color:16777215});a.base2=new THREE.MeshBasicMaterial({color:65280,map:THREE.ImageUtils.loadTexture("images/grid1.jpg")});a.solid=new THREE.MeshBasicMaterial({color:16711795,transparent:!0,opacity:0.05,depthWrite:!1});a.shad_box=new THREE.MeshBasicMaterial({color:15158332,
envMap:this.environment,reflectivity:0.4});a.shad_sphere=new THREE.MeshBasicMaterial({color:2719929,envMap:this.environment,reflectivity:0.4});a.shad_cylinder=new THREE.MeshBasicMaterial({color:9323693,envMap:this.environment,reflectivity:0.4});this.mat=a},addWorker:function(a,b){this.w=new V.Worker(this,a);this.wFun=b||function(){}},addRenderTarget:function(a,b){var c=new THREE.WebGLRenderTarget(a,b,{minFilter:THREE.LinearFilter,magFilter:THREE.LinearFilter,format:THREE.RGBAFormat});this.isWithSerious=
...
return a=6>a.length?"#0"+a:"#"+a};V.PostEffect=function(a,b,c,d){this.shaderLoaded=0;this.root=a;this.name=b;this.isActive=!1;this.init(c);this.callback=d||function(){}};
...
0,0,64,64);this.environment=new THREE.Texture(a);this.environment.needsUpdate=!0;for(var b=this.shaders.length;b--;)this.shaders[b].isActive&&(this.shaders[b].uniforms.env.value=this.environment)}.bind(this);a.src="images/spherical/"+this.envLists[this.nEnv]},add:function(a){this.shaders.push(a)}};
