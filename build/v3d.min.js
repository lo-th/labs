...
null!==this.model&&this.updateSkin()},updateSkin:function(){for(var a,b,c,d=this.reader.Nodes,e=this.bones.length,f,g,k,l=new THREE.Quaternion,h=new THREE.Vector3,n=new THREE.Vector3,m=0;m<e;m++)a=this.bones[m],c=a.name,k=a.parent.matrixWorld,g=a.parent.mtx?a.parent.mtx:k,(b=d[c])?(f=b.matrixWorld.clone(),h.setFromMatrixPosition(f),l.setFromRotationMatrix(f),b=new THREE.Matrix4,a.rootMatrix||(a.rootMatrix=a.matrixWorld.clone()),b.makeRotationFromQuaternion(l),b.multiply(a.rootMatrix),b.setPosition(h),
