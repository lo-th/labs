...
function loop(){
    ...
    //v.tell(v.nav.key.up + "_"+v.nav.key.down+ "_"+v.nav.key.left+ "_"+v.nav.key.right);
    v.render();
}
...
V.Minimap = function(revers, debug){
    ...
    this.miniGlCanvas.style.cssText = 'position:absolute; bottom:10px; right:10px; border:3px solid #74818b;';
    this.miniTop.style.cssText = 'position:absolute; bottom:13px; right:13px;';
    this.mapTest.style.cssText = 'position:absolute; bottom:35px; right:35px;';
    this.mmCanvas.style.cssText = 'position:absolute; bottom:100px; right:10px; border:3px solid #74818b;';
...
V.Player = function( revers, debug ){
	this.revers = revers || false;
...
V.Player.prototype = {
    ...
    addHero:function(m, size){
        ...
        if( this.revers ){
            ...
            this.hero.rotation.y = 180 * V.ToRad;
        }
...
function onload(){
	...
	v.pool.load('droid', onloadNext);
}

function onloadNext(){
	var model = v.pool.meshes.droid.droid;
	var material = new THREE.MeshBasicMaterial({ color:0xFFFFFF, skinning:true, morphTargets:true });
	model.material = material;
	player.addHero(model, 0.01);
}
